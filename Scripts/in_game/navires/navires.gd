...
func _draw():
	...
	if is_selected and player_owner and player_owner.is_human:
		...
		draw_arc(Vector2.ZERO, current_radius, 0, TAU, 32, Color.BLACK, (selection_thickness + 2) * scale_factor)
		draw_arc(Vector2.ZERO, current_radius, 0, TAU, 32, selection_color, selection_thickness * scale_factor)
		...
		draw_arc(Vector2.ZERO, current_radius + base_offset, 0, TAU, 32, glow_color, base_thickness)
		drawable.selection_circle()
	...
	var arrow_base = local_target + Vector2(0, offset_y - arrow_height * arrow_scale)
	var arrow_tip  = local_target + Vector2(0, offset_y)

	# Contour noir
	draw_line(arrow_base, arrow_tip, arrow_outline_color, (arrow_width + 8) * arrow_scale)
	...
	draw_colored_polygon(PackedVector2Array([arrow_tip, left_outline, right_outline]), arrow_outline_color)

	# Flèche principale
	draw_line(arrow_base, arrow_tip, arrow_color, arrow_width * pulse * arrow_scale)
	...
	draw_colored_polygon(PackedVector2Array([arrow_tip, left_point, right_point]), arrow_color)

	# Cercle lumineux
	var glow_color = Color(arrow_color.r, arrow_color.g, arrow_color.b, 0.3)
	draw_circle(arrow_base, 16.0 * pulse * arrow_scale, glow_color)
	draw_circle(arrow_base, 8.0 * arrow_scale, arrow_color)

# =========================
# UI FUNCTIONS
# =========================
	drawable.arrow(local_target)
