...
func _setup_node3d_instance() -> void:
	...
	if pirate and pirate is Node3D:
		...
		DEBUG.log("Navire [%d] - pirateShip ciblé" % id)
	else:
...
func set_input_mode(mode: InputMode) -> void:
	...
	match mode:
		InputMode.MOVE:
			...
			DEBUG.log("Navire [%d] — Mode DÉPLACEMENT" % id)
		InputMode.ATTACK:
			Input.set_default_cursor_shape(Input.CURSOR_CROSS)
			DEBUG.log("Navire [%d] — Mode ATTAQUE" % id)
		InputMode.INSPECT:
			Input.set_default_cursor_shape(Input.CURSOR_HELP)
			DEBUG.log("Navire [%d] — Mode INSPECTION" % id)
		InputMode.NONE:
			...
			if is_selected and current_input_mode != InputMode.NONE:
				match current_input_mode:
					InputMode.MOVE:
						...
						return
					InputMode.ATTACK:
						attempt_shoot(Map_utils.monde_vers_case(mouse_pos))
						set_input_mode(InputMode.NONE)
						get_viewport().set_input_as_handled()
						return
					InputMode.INSPECT:
...
func get_ships_at_position(target_case: Vector2i) -> Array[Navires]:
	...
	if data and data.has_method("getNavireByPosition"):
		for ship in data.getNavireByPosition(target_case):
...
func get_ship_at_position(pos: Vector2) -> Navires:
	for ship in get_tree().get_nodes_in_group("ships"):
		if ship is Navires and ship != self:
			if pos.distance_to(ship.global_position) <= ship.interaction_radius:
				return ship
	return null
...
func _process_movement(delta: float) -> void:
	...
	if distance <= snap_threshold:
		...
		if path.is_empty():
			...
			show_arrow = false
			queue_redraw()
			DEBUG.log("Navire [%d] destination atteinte" % id)
	else:
...
func _update_fog_of_war() -> void:
	...
	if not fog_manager:
		fog_manager = get_tree().get_first_node_in_group("fog_manager")
	if fog_manager and fog_manager.has_method("force_update"):
		fog_manager.force_update()
		return
	var fog_of_war = get_tree().get_first_node_in_group("fog_of_war")
	if fog_of_war and fog_of_war.has_method("reveal_around_position"):
...
func try_start_fishing() -> void:
	...
	if not fish_manager.is_fish_tile(case_actuelle):
		DEBUG.log("Navire [%d] - Pas une zone de pêche" % id)
		return
	if not fish_manager.can_fish_at(case_actuelle):
		DEBUG.log("Navire [%d] - Zone épuisée !" % id)
	...
	stats_panel.show_ally()
	DEBUG.log("Navire [%d] - Pêche sur case %s" % [id, case_actuelle])
...
func finish_fishing() -> void:
	...
	if fish_feedback_label:
		...
		sig_show_stats.emit()
	DEBUG.log("Navire [%d] - Pêche terminée : +%d poissons" % [id, gain])
#endregion peche
